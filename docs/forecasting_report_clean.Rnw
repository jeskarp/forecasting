...
The methods of forecasting disease incidence that were evaluated in this work were developed by members of the R Epidemics Consortium. The empirical outbreak linelists were openly accessible from Open Data Sierra Leone (Ebola), and the \textit{EpiEstim} (SARS) and \textit{outbreaks} (influenza) R packages.
...
Branching process models are often used for forecasting disease incidence in real-time to aid outbreak response. In this work, we set out to evaluate the performance of such models for early outbreak analysis purposes. Three empirical outbreaks, Ebola, influenza, and SARS, and 90 simulations based on the $R_{0}$ and serial interval distribution of the real outbreaks were created. A branching process model was used to predict disease incidence at different stages of the outbreak. The performance of the projected daily incidence was explored through four metrics: the average residual, mean-square error, sharpness, and bias. My work suggested that in general the branching process model tends to overestimate the forecasted daily incidence, though forecasts do improve with increasing amounts of observed data and decreasing projection length. \textcolor{red}{end with a general guideline sentence and make less dry} 
...
\end{equation}
where $b$ represents the regression slope for each variable, $\epsilon$ is the error, and $a$ is the intercept. 
...
@
\caption{The medians and 95\% quantile ranges of the four prediction metrics - average residual, mean-square error (MSE), sharpness, and bias, for Ebola-like (left), influenza-like (middle), and SARS-like (right) outbreaks for projection window's distance from the calibration window increases from 1 to 4 for calibration windows of size 1 (dark blue), 2 (blue), and 4 (light blue).}
...
lm_table[c(3, 10, 18, 25), 2] <- "Disease \\textsuperscript{2}" # Ebola, influenza, SARS
lm_table[c(4, 11, 19, 26), 2] <- "No. cases \\textsuperscript{3}" # "No. cases in calibration window"
lm_table[c(5, 12, 20, 27), 2] <- "Proj. window \\textsuperscript{4}" # "Projection window no."
lm_table[c(6, 13, 21), 2] <- "Pred. group \\textsuperscript{5}"# "Prediction group type"
lm_table[c(14), 2] <- "Ratio \\textsuperscript{6}" # "Ratio of calibration window size to projection window number"
lm_table[c(7, 15, 22, 28), 2] <- "Interaction \\textsuperscript{7}" # "Interaction between calibration and projection window"
...
\\textsuperscript{1} calibration window size: 1-7 mean serial intervals \\newline 
\\textsuperscript{2} disease: Ebola, influenza, or SARS \\newline
\\textsuperscript{3} number of cases in calibration window \\newline 
\\textsuperscript{4} projection window: the distance in mean serial intervals between the end of the calibration window and the end of the projection window \\newline 
\\textsuperscript{5} type of prediction: all trajectories do not predict new cases, all trajectories predict new cases, some trajectories predict new cases  \\newline 
\\textsuperscript{6} ratio of calibration window size to projection window number \\newline 
\\textsuperscript{7} additive interaction between calibration window size and projection window} \\\\\n")

tab <- xtable(lm_table, caption = "The coefficients of the best-performing linear regression models for explaining variation in the average residuals, mean-square error, sharpness, and bias in the forecasts of simulated outbreaks of Ebola, Severe Acute Respiratory Syndrome (SARS), and influenza. CI: confidence interval, MSE: mean-square error.", label = "metric_lm_table")
...
\subsubsection{Inferring the effective reproduction number}
First the observed daily incidence (cases per day) and known estimates of the serial interval distribution were used to obtain a likelihood distribution for the effective reproduction number, $R$. Here it is assumed that the serial interval follows a discretised Gamma distribution. 
...
The same information can also be used to estimate the force of infection, calculated as the sum of individual forces of indection for a given day $t$:

% EpiEstim overall_infectivity
\begin{equation}
\lambda_{t} = \sum_{k = 1}^{t-1}I_{t - k}w_k
\end{equation}
Here $w$ is the discretised serial interval distribution.

The likelihood of different values of $R$ based on the computed force of infection can then be calculated:

% sum(stats::dpois(x[-1], lambda = R * x_lambdas, log = TRUE))
% x is daily incidence
% x_lambdas comes from overall_infectivity
\begin{equation}
P(R) = \sum_{t = 1} Pois(x_{t} | R * \lambda_{t})
\end{equation}

\textcolor{red}{sample\_R}
This created likelihood distribution of $R$ can be sampled from to obtain plausible values of $R$ for forecasting incidence. 

\subsubsection{Projecting incidence}
...
The combination of observed daily incidence, serial interval distribution, and sample of plausible values of $R$ can then be used to forecast daily incidence using a branching process. Here daily incidence follows a Poisson process, which in turn is determined by the daily infectiousness at time $t$, $\lambda_{t}$. $\lambda_{t}$ can be calculated as: 
% projections
\begin{equation}
\lambda_{t} = \sum_{s = 1}^{t-1}y_{s}w(t-s)
\end{equation}
where the sum of product of the incidence at time $s$ and the probability mass function of the serial interval, $w$, for the time period between time $t$ and $s$ is taken.
...
For the following projection days, the forecasted daily incidence during the previous infection days is taken into account.
...
\begin{figure}[h]
<<whole_residual, echo = FALSE, cache = FALSE, fig.width = 8, fig.height = 6.5, warning = FALSE>>=
...
\subsubsection{MSE}
\begin{figure}[h]
<<whole_mse, echo = FALSE, cache = TRUE, fig.width = 8, fig.height = 6.5, warning = FALSE>>=
...
\begin{figure}[h]
<<whole_sharpness, echo = FALSE, cache = TRUE, fig.width = 8, fig.height = 6.5, warning = FALSE>>=
...
\subsubsection{Bias}
\begin{figure}[h]
<<whole_bias, echo = FALSE, cache = TRUE, fig.width = 8, fig.height = 6.5, warning = FALSE>>=
...
\subsection{Simulated outbreaks: model comparison}
...
# model names
aic_table[c(1, 5, 9), 2] <- "Basic \\textsuperscript{1}"
aic_table[c(13), 2] <- "Basic \\textsuperscript{2}"
aic_table[c(2, 6, 10, 14), 2] <- "Basic + ratio \\textsuperscript{3}"
aic_table[c(3, 7, 11, 15), 2] <- "Basic + interaction \\textsuperscript{4}"
aic_table[c(4, 8, 12, 16), 2] <- "Basic + ratio + interaction \\textsuperscript{5}"
...
\\textsuperscript{2} A basic model consisting of calibration window size, disease (Ebola, influenza, SARS), number of cases in the calibration window, projection window number, type of predictions in the projection window (all projections show no cases, some projections with no cases, all projections show cases) \\newline 
\\textsuperscript{3} The basic model and ratio of calibration window size to projection window number  \\newline 
\\textsuperscript{4} The basic model and additive interaction between calibration window size and projection window number \\newline 
\\textsuperscript{5} The basic model incorporation the ratio between windows and additive interaction \\newline} 
\\\\\n")
...
\newpage
<<data_sharpness_table, eval = FALSE, echo = FALSE, results = "asis">>=
library(xtable)
...
<<data_bias_table, eval = FALSE, echo = FALSE, results = "asis">>=
library(xtable)
...
<<case_plots, eval = FALSE, echo = FALSE, eval = FALSE>>=
...
% \subsubsection{Average residual}
\begin{figure}[h]
<<residual_plot_total, eval = FALSE, echo = FALSE, fig.width = 8, fig.height = 3>>=
library(ggplot2)
text_size <- 5
...
<<data_residual_table, eval = FALSE, echo = FALSE, eval = FALSE, results = "asis">>=
library(xtable)
