...
The model used by this $R$ inference method describes daily incidence $x_{t}$ as:
\begin{equation}
x_{t} \sim \mathcal{P}(\lambda_{t})
\end{equation}
where the total force of infection for a given day, $\lambda_{t}$, determines the Poisson process $P()$ \citep{Cori2013, Jombart2017}.
