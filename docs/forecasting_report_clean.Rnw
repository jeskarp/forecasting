...
The future incidence for each real and simulated outbreak was forecasted for a given number of time windows by first estimating the reproduction number ($R$) for the outbreak so far using the \textit{get\_R} function in the \textit{earlyR}-package \citep{Jombart2017}\ref{projection_ex}. Future dates' incidence could then be projected based on the likelihood distribution of this $R$ using the \textit{project} function of the \textit{projections}-package \citep{Jombart2018}.
...
@
\caption{Examples of daily incidence forecasts for simulations of Ebola-like outbreaks. Daily incidence curves (black histogram) are shown with the forecasted daily incidence (shaded blue areas), along with the median projected incidence (purple line) and 95\% interval (pink line). Figure A shows a well-performing projection where the forecasted daily incidence along with its 95\% interval is near the true observed incidence. Figure B shows a projection where the median forecasted incidence is near the true observed values, but the spread of projections is wide. Figure C shows a projection where the median forecasted simulation is not near the true observed incidence and the spread of forecasts is wide.}
\label{good_bad_plot}
\end{figure}
...
@
\caption{How the medians and interquartile ranges of the four prediction metrics - average residual, mean-square error, sharpness, and bias, for Ebola-like, influenza-like, and SARS-like outbreaks differ as the projection window's distance from the calibration window increases from 1 to 4 for calibration windows of size 1 (dark blue), 2 (blue), and 4 (light blue).}
...
lm_table[c(2, 9, 17, 24), 2] <- "Cal. size"# "Calibration window size"
lm_table[c(3, 10, 18, 25), 2] <- "Disease" # Ebola, influenza, SARS
lm_table[c(4, 11, 19, 26), 2] <- "No. cases" # "No. cases in calibration window"
lm_table[c(5, 12, 20, 27), 2] <- "Proj. window" # "Projection window no."
lm_table[c(6, 13, 21), 2] <- "Pred. group"# "Prediction group type"
lm_table[c(14), 2] <- "Ratio" # "Ratio of calibration window size to projection window number"
lm_table[c(7, 15, 22, 28), 2] <- "Interaction" # "Interaction between calibration and projection window"

# Estimates and confidence intervals
for (i in 1:7){
lm_table[i, 3] <- paste(round(residual_summary$coefficients[i, 1], 2), "(", round(residual_summary$coefficients[i, 1] - 1.96 * residual_summary$coefficients[i, 2], 2), "-", round(residual_summary$coefficients[i, 1] + 1.96 * residual_summary$coefficients[i, 2], 2), ")", sep = "")
}

for (i in 1:8){
lm_table[(i + 7), 3] <- paste(round(mse_summary$coefficients[i, 1], 2), "(", round(mse_summary$coefficients[i, 1] - 1.96 * mse_summary$coefficients[i, 2], 2), "-", round(mse_summary$coefficients[i, 1] + 1.96 * mse_summary$coefficients[i, 2], 2), ")", sep = "")
}

for (i in 1:7){
lm_table[(i + 15), 3] <- paste(round(sharpness_summary$coefficients[i, 1], 2), "(", round(sharpness_summary$coefficients[i, 1] - 1.96 * sharpness_summary$coefficients[i, 2], 2), "-", round(sharpness_summary$coefficients[i, 1] + 1.96 * sharpness_summary$coefficients[i, 2], 2), ")", sep = "")
}

for (i in 1:6){
lm_table[(i + 22), 3] <- paste(round(bias_summary$coefficients[i, 1], 2), "(", round(bias_summary$coefficients[i, 1] - 1.96 * bias_summary$coefficients[i, 2], 2), "-", round(bias_summary$coefficients[i, 1] + 1.96 * bias_summary$coefficients[i, 2], 2), ")", sep = "")
}
...
colnames(lm_table) <- c("Metric", "Coefficient", "Estimate (95% CI)", "p-value", "Adjusted R-squared")
...
print(tab, hline.after=c(-1, 0, 7, 15, 22, 28), comment = FALSE, math.style.exponents = FALSE, include.rownames = FALSE, caption.placement = "bottom", type = "latex", sanitize.rownames.function = identity, tabular.environment = "tabularx", width = "\\textwidth", size = "\\small")
@
...
\newpage
\section{Discussion}
\textcolor{red}{Thibaut and I didn't discuss my Discussion much either, but he would like my discussion to answer questions such as "When does the model work?" "When didn't it work so well" "Are we good at estimating uncertainty?" "Are there any systematic biases?" "I have X amount of data - how far can I project? Can I trust what I have projected for the next serial interval"}
...
\newpage
\subsection{Simulated outbreaks: distribution of metrics}
\begin{figure}[h]
...
\FloatBarrier
\newpage
\subsection{Simulated outbreaks: zero-incidence}
\begin{figure}[h]
...
residual_massive <- ggplot(real_total_table, aes(cali_window_size, y = residual, color = dataset)) +
                    ...
                    geom_hline(yintercept = 0, linetype = "dashed") +
                    geom_point(size = 1.5, shape = 16) +
                    facet_wrap(dataset~proj_window_no, scales = "free") +
                    labs(y = "Residual", x = "Calibration window size") +
                    coord_cartesian(xlim = c(1, 7)) +
                    scale_x_discrete(labels = c("1", "2", "3", "4", "5", "6", "7"), limits =  c("1", "2", "3", "4", "5", "6", "7")) +
                    theme(legend.position = "none") # +
