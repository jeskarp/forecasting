...
Empirical daily incidence data for three diseases, Ebola, influenza, and SARS, were chosen to assess the performance of the branching process model \citep{Leone2014, Cori2018, Jombart2018a}. The early Ebola outbreak data analysed here was that of the 2014 outbreak in Sierra Leone \citep{Leone2014}. The analysed early influenza outbreak data was from the H1N1 outbreak of 1918 \citep{Cori2018}. The early outbreak data for SARS, on the other hand, was from an outbreak in Canada in 2003 \citep{Cori2018}. Table \ref{outbreak_table} shows the published $R_{0}$, and the mean and standard deviation (in days) of the serial interval for these empirical outbreaks.

For each given disease, 80 simulated outbreaks were run based on the published $R_{0}$  serial interval (in days) for each empiricial disease outbreaks (Table \ref{outbreak_table}). Each simulation was run for eight times the rounded mean serial interval for a population size of 1 million. The length of the simulation was therefore 96 days for Ebola, 24 days for influenza, and 72 days for SARS. The simulations were conducted using the \textit{simOutbreak} function from the package \textit{outbreaker} \citep{Jombart2014}. For projection purposes, a section of this data would later be used to calibrate the predictive branching process model while other sections would be hidden to test the performance of said model.
...
outbreak_table[3, 2] <- 2.7
outbreak_table[2, 2] <- 1.77

# Disease SI mean
outbreak_table[1, 3] <- 11.6
outbreak_table[3, 3] <- 8.7
outbreak_table[2, 3] <- 2.6

# Disease SI SD
outbreak_table[1, 4] <- 5.6
outbreak_table[3, 4] <- 3.6
outbreak_table[2, 4] <- 1.5

# Source
outbreak_table[1, 5] <- "\\citep{Team2014}" # "WHO 2014, SL data"
outbreak_table[3, 5] <- "\\citep{Fraser2011}" # "EpiEstim, Fraser (2011)"
outbreak_table[2, 5] <- "\\citep{Campbell2018}" # "Campbell et al. (2018)"
...
% The full details on how the forecasting branching process model evaluated in this study works can be found in Supplementary Information section 1.1. The information required for forecasting are data on observed daily incidence and a discrete Gamma-distributed serial interval for the disease in question.
...
To forecast daily disease incidence, a branching process forecasting method based on Cori et al.'s method for estimating $R$ was used \citep{Cori2013, Jombart2018}. Before forecasting, the observed daily incidence (cases per day) and known estimates of the serial interval distribution were used to obtain a maximum likelihood estimate for the effective reproduction number, $R$. Here it is assumed that the serial interval follows a discrete Gamma distribution.

The model used by this $R$ inference method describes daily incidence $x_{t}$ as a Poisson process, $\mathcal{P}()$ determined by the total force of infection $\lambda_{t}$  \citep{Cori2013, Jombart2017}:
\begin{equation}
x_{t} \sim \mathcal{P}(\lambda_{t})
\end{equation}

The force of infection for a day $t$ is calculated as the sum of individual forces of infection for that day:
\begin{equation}
\lambda_{t} = R \times \sum_{k = 1}^{t}x_{k}w(t - k)
\end{equation}
Here $w()$ is the discrete serial interval distribution (probability mass function) for day $k$ and $x$ is the incidence on a given day \citep{Jombart2017}.
...
This can then be used to obtain a maximum-likelihood estimation for $R$ and the range of $R$ likelihoods can be sampled from to obtain plausible values of $R$. This $R$ inference method is implemented in the $earlyR$ package \citep{Jombart2017}. 

The combination of observed daily incidence, serial interval distribution, and the previously calculated sample of plausible values of $R$ can then be used to forecast daily incidence using a branching process. As for the $R$ estimation, the forecasted daily incidence follows a Poisson process, determined by the daily infectiousness at time $t$, $\lambda_{t}$ (Eq. 1). $\lambda_{t}$ can also be calculated in the same manner as the $\lambda_{t}$ for a window of observed incidence (Eq. 2). As $R$ remains constant throughout the projection, it is drawn only once from the sample of plausible values of $R$ \citep{Jombart2018}. For the following projection days, the daily incidence of the previous days of the forecast are taken into account when re-calculating the force of infection \citep{Jombart2018}. In R, this projection method can be implemented with the $project$-function of the $projections$ package \citep{Jombart2018}.
...
The forecasts were run for different numbers of days depending on disease. A single time window was defined as one rounded mean serial interval, in days. The mean serial interval was chosen rather than a time window of constant size for all diseases to make the analysis of the forecasts' performance more comparable across diseases with differing serial intervals. This means that a single time window for Ebola and Ebola-like simulations had a time window of 12 days, influenza 3 days, and SARS 9 days. Each single projection window within a full forecast was the size of a single time window, while calibration windows were multiples of the time window, with all calibration windows starting from the very beginning of the outbreak (Fig. \ref{projection_ex}). The differing calibration window lengths are due to the outbreak's force of infection for a given time window being affected by the cases from previous time windows. Incidence data was hidden for dates occurring after the calibration window.

The maximum total length of the forecast was 4 time windows, while the calibration window ranged in size from 1 time window to 7 time windows. The forecast length and calibration window sizes were combined so that for a calibration window size of 1 to 4, the forecast length was 4 time windows. For calibration window sizes of 5, 6, and 7 time windows, the forecast lengths were 3, 2, and 1 time windows, respectively. This limitation in the total number of time windows used for calibration and forecasting to a maximum of 8 was done in an attempt to keep the focus on the performance of the branching process model during early outbreak analysis and avoid the simulated outbreaks reaching a point where the outbreak has burned through the susceptibles.  
...
The performance of different calibration windows with varying projection windows was quantified by observing the average residual, mean-square relative error (MSE), sharpness, and bias of the projections for each projection window of each simulated or real outbreak. Of these metrics, the average residual is a well-established method of measuring error, while the usage of MSE was inspired from Viboud et al., and the sharpness and bias metrics are adapted from Funk et al. \citep{Viboud2017, Funk2018}.  
...
%  order to obtain a forecast of daily incidence, first the force of infection for the observed time window is calculated, which was then used to calculate the likelihoods of varying values of the effective reproduction number, $R$. This $R$ likelihood distribution was then sampled from to obtain plausible values of $R$ used for projection purposes. This in turn is then used to predict force of infection and thus the number of new cases.
...
% Before forecasting daily incidence, a maximum likelihood estimate of the effective reproduction number, $R$, was calculated by considering how likely the observed incidence would be given a value of $R$. To do so, it was assumed that the daily incidence follows a Poisson process  This process of estimating a likelihood was repeated for many $R$ values, the range of which are sampled to obtain plausible values of $R$ for the forecasts. 
%where the force of infection, $\lambda_{t}$, for the observed time window was calculated. $\lambda_{t}$ was then used to calculate a maximum likelihood estimate and a likelihood distribution for the effective reproduction number, $R$, for the window of observed incidence \citep{Jombart2017}. This distribution was then sampled from to obtain plausible values of $R$ used for projection purposes \citep{Jombart2017}. This in turn is then used to predict force of infection and thus the number of new cases.
...
% projections
% \begin{equation}
% \lambda_{t} = \sum_{k = 1}^{t-1}x_{k}w(t-k)
% \end{equation}
...
As sharpness is measured by observing how far from the median predicted incidence a single stochastic trajectory is, a forecast's sharpness can be expected to be high for a forecast where many of its individual stochastic trajectories forecast no new cases. This is because the median predicted incidence of these trajectories will be zero and thus many trajectories will be exactly on the median. This in turn has an effect on the distribution of the values that sharpness takes, where forecasts that include the possibility of no new cases either have a high sharpness or sharpness that is lower than that of forecasts that do not predict extinction (SI Fig. \ref{sharpness_type_plot}). The drop in sharpness seen as calibration size increases is partly explained by the increasing number of projections that have rejected the possibility of no cases occurring in the projection window as more trajectories deviate from the median at some point during the course of the projection (SI Fig. \ref{cali_zero_plot}). 
...
# Number diseases
total_table$disease_num <- 3
total_table$disease_num[total_table$disease == "influenza"] <- 2
total_table$disease_num[total_table$disease == "sars"] <- 1
...
# sharpness
lm_table[20, 4] <- "\\footnotesize{<}2.00e-16" # signif(sharpness_summary$coefficients[1, 4], 3)
...
lm_table[24, 4] <-"\\footnotesize{<}2.00e-16" # signif(sharpness_summary$coefficients[5, 4], 3)
...
\\footnotesize{\\textsuperscript{2} disease: SARS (baseline), influenza (2), or Ebola (3)} \\newline
...
\\footnotesize{\\textsuperscript{5} type of prediction: all trajectories do not predict new cases (3), all trajectories predict new cases (2), some trajectories predict new cases (baseline)} \\newline 
...
align(tab) = c("p{0.1\\textwidth}", "p{0.1\\textwidth}", "p{0.18\\textwidth}", "p{0.37\\textwidth}", "p{0.12\\textwidth}", "p{0.15\\textwidth}")
...
@
\FloatBarrier
Table \ref{metric_lm_table} presents the most parsimonious linear regression models with an addition of interaction terms for each of the four performance metrics. A comparison of the AICs of all the linear models can be found in Supplementary Information Table \ref{sim_metric_aic_table}. For the average residual, sharpness, and bias, the best model was one which contained all the explanatory variables and an additive interaction term between calibration window size and projection window number. For the MSE, the best model additionally included the ratio of calibration window size to projection window number. Out of the metrics, the change in sharpness due to the explanatory variables is best explained by the linear regression model with an adjusted R-squared of 0.49, while the MSE was least explained with an adjusted R-squared of 0.05. 

Increasing calibration window size tended to reduce the average residual and increase sharpness and the MSE (Table \ref{metric_lm_table}). This in general suggests that overestimating incidence is more likely when calibration windows are larger, while the range of forecasts becomes narrower. Meanwhile, increasing the number of cases in the calibration window reduced the average residual and increased the MSE, but reduced sharpness and increased bias (Table \ref{metric_lm_table}). The reduction in sharpness could be due to there being fewer stochastic trajectories forecasting an incidence of zero as the number of cases in the calibration window increase, meaning that trajectories will more easily deviate from the median trajectory (SI Fig. \ref{sharpness_type_plot}, \ref{cali_zero_plot}). Having more cases in the calibration window could also increase the chances of a greater overprediction, as a higher incidence incidence in the observation window could translate to greater forecasted incidence, and overestimating by a greater margin is easier when larger incidences are predicted. 

Similarly, the average residual and sharpness decreased with increasing projection window number while MSE and bias increased (Table \ref{metric_lm_table}). This indicates that as the projection window of interest is further in the future, the forecasts are more prone to overestimating incidence and the range of forecasts increases.

Projections that did not contain any zero-incidence trajectories had a higher sharpness and lower bias than projections that contained some trajectories with zero-incidence, while projections that only contained zero-incidence had a lower sharpness and higher bias.

% As the interaction term between calibration window size and projection window number is statistically significant, it suggests that the effect of the projection window number is different for each of the calibration window sizes.

The correlation coefficient of the MSE and average residual was \Sexpr{round(res_mse_cor$estimate, 2)}, which means that MSE tends to decrease with increasing average residual. Bias also decreases with increasing average residual with a correlation coefficient of \Sexpr{round(res_bia_cor$estimate, 2)}. Additionally, bias is positively correlated with MSE with a correlation coefficient of \Sexpr{round(mse_bia_cor$estimate, 2)}. All three metrics, the average residual, MSE, and bias, are affected by either the extent or the direction of the over- or underprediction of daily incidence. Therefore, it is reasonable to assume that as the value of an average residual becomes slightly less negative as a sign of an overpredicting forecast improving, bias also moves from towards zero and becomes slightly less positive. The extent of this overprediction decreases as the average residual increases, explaining why MSE also reduces. 

Sharpness, on the other hand, increases as the average residual increases with a correlation coefficient of \Sexpr{round(res_sha_cor$estimate, 2)} and decreases with increasing MSE (\Sexpr{round(mse_sha_cor$estimate, 2)}), while bias is negatively correlated with sharpness with a correlation coefficient of \Sexpr{round(sha_bia_cor$estimate, 2)}. This suggests that as the forecast accuracy improves, seen as the average residual, MSE, and bias moving toward 0, the forecast's also forecasts a narrower range of projections. 
...
@
\caption{How the four performance metrics for empirical Ebola, influenza, and SARS outbreaks differ as the projection window's distance from the calibration window increases from 1 to 4 for calibration windows of size 1 (dark blue), 2 (blue), and 4 (light blue).}
\label{all_real_metric_plot}
\end{figure}

As can be seen in Figure \ref{all_real_metric_plot}, the branching process model fared worse at forecasting daily incidence for the empirical outbreaks than for the simulated outbreaks. This is exhibited by the scale of differences in the average residual and MSE for different calibration and projection windows. For example, the empirical Ebola outbreak's average residual and MSE estimates for a calibration size of 1 mean serial interval and a projection window number of 4 are approximately 9 times and 3 times greater respectively than the more negative end of the 95\% quantile range for the corresponding simulated Ebola-like outbreaks (Fig. \ref{all_metric_plot}, \ref{all_real_metric_plot}).  As with the simulated outbreaks, the full performance metrics for each calibration window size and projection window size combination is presented in the Supplementary Information (section 3.2).

The patterns of performance of the branching process model in forecasting incidence for the SARS outbreak is different than those of the Ebola and influenza outbreaks Figure \ref{all_metric_plot}. The early SARS outbreak analysed here is one which seemed to have reached its peak though beyond the scope of the early outbreak analysis covered in this study, another peak of cases can be observed in the full dataset (\ref{Cori2014}). As based on the observation window in this study it seems that the peak of the outbreak is captured in the calibration windows, which is not well-captured by the branching process model. The Ebola and influenza outbreaks analysed in this study were more clearly still in the initial phase of the outbreak for the dates of data that were used here, where daily incidence may still be increasing. 

As the model was not forecasting zero-incidence as often for the empirical outbreaks as for the simulated outbreaks, the forecasts do not often reach a sharpness of 1 (Fig. \ref{all_real_metric_plot}, SI Fig. \ref{real_whole_sharpness}). For Ebola and influenza, sharpness decreases as the projection window moves further from the calibration window, reflecting a widening range of stochastic trajectories for daily incidence. Additionally, sharpness is seen to increase with increasing calibration window size. Sharpness starts off high for SARS as a forecast of zero-incidence is a possibility. It then decreases with increasing projection window numbers as the forecast moves further into the future. From here, the sharpness of the forecasts increases as forecasts move to later dates.

Compared to the simulated outbreaks, the empirical Ebola and influenza outbreaks were overestimating the number of cases in the projection window more consistently, as can be deduced from observing the bias metric for Ebola and influenza (Fig. \ref{all_real_metric_plot}). Where simulated Ebola and influenza outbreaks generally had bias medians between 0 and 0.5, the empirical outbreaks' bias is near 1 for many Ebola outbreak forecasts and some influenza outbreak forecasts(Fig. \ref{all_metric_plot}, \ref{all_real_metric_plot}, SI Fig. \ref{real_whole_bias}). The overestimation tends to increase as projection windows are further from calibration windows, as was also observed for the simulated outbreaks. The SARS outbreak's forecasts, on the other hand, underestimate incidence for the earlier calibration windows but then starts to overestimate incidence after the third calibration window. 
...
Overall, the analysis of daily incidence forecasts based on simulated Ebola-like and influenza-like outbreaks found that a forecast is more trustworthy the longer the calibration window is and the closer projection window of interest is to the calibration window (Fig. \ref{all_metric_plot}). This same trend holds for the empirical Ebola and influenza outbreaks as well (Fig. \ref{all_real_metric_plot}).   

% Discussion of performance metrics 
While the branching process model was better at forecasting daily incidence for simulated outbreaks according to the residual, MSE, and bias, the sharpness was not as much affected for the empirical outbreaks' forecasts (Fig. \ref{all_metric_plot}, \ref{all_real_metric_plot}). The better performance of the simulated outbreaks could be due to them representing an ideal scenario where every case is reported, unlike the empirical outbreaks for which the outbreak started at a point where there were already cases every day or nearly so and for which the extent of case reporting is not known (SI Fig. \ref{empirical_plot}). 

Due to the simulated outbreaks having perfect reporting and including the sporadic cases that occurred in the first few days of the outbreak, the forecasts suggested more uncertainty at the beginning of the outbreaks as to whether the outbreak would go extinct or not (SI Fig. \ref{cali_zero_plot}). This could have contributed to their forecasts having a wider scope of possible scenarios, shown by a wide range of outcomes according to the stochastic trajectories of the projection and widening 95\% quantile ranges for the performance metrics as projection window numbers increased.

Out of the performance metrics used in this study, the MSE may have been superfluous considering that the same trends could be seen with the average residual, that the two metrics were highly correlated, and that the MSE does not take into account whether a forecast is over- or underpredicting, but rather the extent to which the forecast is inaccurate. Additionally, sharpness as a performance metric might be more informative for later stage outbreaks or outbreaks where the forecast suggests that the chances of the outbreak dying off are slim. For the simulated outbreaks, it may have been too early to evaluate sharpness in a coherent matter, as the forecasts were predicting the outbreak to die out.

While the branching process model had a tendency of overestimating the forecasted incidence of simulated outbreaks in general, this effect was highlighted for early calibration windows compared to later calibration windows and later projection window numbers in comparison to ones closer to the calibration window (Fig. \ref{all_metric_plot}). The overestimation does reduce with larger calibrarion window size (Table \ref{metric_lm_table}). This could be due to there being more days' worth of cases or due to the outbreak, if it did not go extinct, being more established. Whether the overestimation would be reduced if calibration window sizes would all be of the same length is a question that cannot be answered with the current model, as the method of calculating force of infection requires the contribution of the individual forces of infection of cases from earlier on in the outbreak. It is possible that Ebola-like outbreaks were observed to have the greatest overestimations in forecasted incidence due to them also having the largest outbreak sizes (SI Fig. \ref{sim_dist}). This is because metrics such as the average residual consider the absolute difference between the projection and true data rather than the relative difference.
...
% Limitations
One of the limitations of this study is that the performance of the forecasting method is mostly evaluated under ideal circumstances on simulated outbreaks that have been created using a similar model. This means that abnormalities, such as unexpected growth curves, are not a concern and are not considered. Regardless, as the performance of the forecasting method used in this study has not been evaluated before, it can be considered reasonable to start from evaluating its performance for the ideal scenario. There is also a loss of forecast performance information when evaluating time windows rather than individual days. This could mean that there may be trends in forecast performance within the time windows that have gone unobserved.
...
\begin{figure}[h]
\begin{center}
<<cali_zero_table, echo = FALSE, fig.width = 8, fig.height = 9, out.width=".7\\textwidth">>=
facet1_names <- c(
  "1" = "Calibration 1",
  "2" = "Calibration 2",
  "3" = "Calibration 3",
...
residual_massive
@
\caption{The average residual for all seven calibration window sizes presented separately for each projection window number and disease, as indicated by the labels on top of each panel of empirical outbreaks of Ebola (red), influenza (green), and SARS (blue).}
...
mse_massive
@
\caption{The mean-square error for all seven calibration window sizes presented separately for each projection window number and disease, as indicated by the labels on top of each panel of empirical outbreaks of Ebola (red), influenza (green), and SARS (blue).}
...
sharpness_massive
@
\caption{Sharpness for all seven calibration window sizes presented separately for each projection window number and disease, as indicated by the labels on top of each panel of empirical outbreaks of Ebola (red), influenza (green), and SARS (blue).}
...
bias_massive
@
\caption{Bias for all seven calibration window sizes presented separately for each projection window number and disease, as indicated by the labels on top of each panel of empirical outbreaks of Ebola (red), influenza (green), and SARS (blue).}
